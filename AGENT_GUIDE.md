...
| **Video generation** | `open-montage` (mandatory process for client deliverables and numbered shot lists: per-shot contract, reference anchors, one shot per call, adherence scorecard), `seedance-2-5` (the 2.5 prompt contract: section order, locks, assets), `seedance-2-0` (preferred premium default — cinematic, trailer, multi-shot, synced audio, lip-sync), `gemini-omni` (conversational video editing, reference tags, timecoded beats), `ai-video-gen`, `ltx2` |
